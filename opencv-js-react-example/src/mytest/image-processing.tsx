...
export function ImageProcessing() {
  ...
  const perspectiveTransformation = () => {
    if(canvasRef.current){
      const src: Mat =  cv.imread(canvasRef.current);
      ...
      let srcTri = cv.matFromArray(4, 1, cv.CV_32FC2, [56, 65, 368, 52, 28, 587, 389, 390]);
      let dstTri = cv.matFromArray(4, 1, cv.CV_32FC2, [0, 0, 300, 0, 0, 300, 200, 300]);
